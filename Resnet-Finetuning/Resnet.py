...
class ResNetModel(object):
    ...
    def __init__(self, x, is_training, depth=50, num_classes=2):
        ...
        self.depth = depth
        #self.fc1 = tf.Variable([128,1024], name="fc1")
    ...
    def inference(self):
        ...
        #    self.fc1 = fc(self.avg_pool, num_units_out=2048,name='fc1')

        with tf.variable_scope('fc'):
            self.prob,self.weights = fc(self.avg_pool, num_units_out=self.num_classes,name='fc')

        #return  self.prob#,self.fc1

    def loss(self, batch_x, batch_y=None):
        y_predict = self.prob#inference(batch_x)# 
        weights = tf.constant(0.60) 
        ...
        cross_entropy = tf.nn.softmax_cross_entropy_with_logits(logits=y_predict, labels=batch_y)
        #cross_entropy= tf.nn.weighted_cross_entropy_with_logits(logits=y_predict, targets=batch_y, pos_weight=weights)
    ...
    def load_original_weights(self, session, skip_layers=[],weight_Path='ResNet-L50.npy'):
...
def fc(x, num_units_out,name):
    with tf.variable_scope(name) as scope:
        ...
        return act, weights#tf.nn.xw_plus_b(x, weights, biases)
